...
test("full access user can use automatic lock", async ({ page }) => {
  const fullAccessEmail = `full-access-auto-${Date.now()}@example.com`;
  ...
  await page.getByPlaceholder("Enter user name").fill("Full Access Auto Lock Test");
  ...
  // Full Access can use Quick Controls / Automatic Lock
  await page.getByRole("button", { name: "Settings" }).click();
